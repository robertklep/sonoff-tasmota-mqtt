...
module.exports = class Device extends EventEmitter {
  ...
  async getSensors(timeout = null) {
    // GPIO-based sensors
    let sensors = await this.sendCommand('gpio').waitFor('result', timeout);
    let names   = Object.keys(sensors)
    ...
    // I2C-based sensors
    if (names.includes('I2C SDA')) {
      try {
        // Have to wait for first 'SENSOR' message to be published.
        sensors = await this.waitFor('sensor', timeout);
        delete sensors.Time;
        delete sensors.TempUnit;
        names.push(...Object.keys(sensors));
      } catch(e) {}
    }
    return names;
  }
